 ...
 * {@link #getLastPopulation()} and {@link #getNextPopulation()}. Past
...
public class GeneticAlgorithm<T extends Chromosome> {
     ...
     * evolution
     */
    private Population<T> lastPopulation = null;
    private Population<T> nextPopulation = null;
    /** This field is deprecated and being replaced by lastPoulation */
    ...
    /**
     * Returns the current population. This is the current population reference. 
     * When the algorithm starts, it is a reference to the initial population.
     * Deprecated. Make use of {@link #getLastPopulation()} instead.
     * <p>
     *
     * @return the current population.
     * @deprecated Use {@link #getLastPopulation()} instead.
     */
     ...
     * starts, it is empty, and it is eventually affected by the randomization process.
     * <p>
     *
     * @return the last population processed so far.
     */
    public final Population<T> getLastPopulation() {
    ...
    public final void evolve(boolean restart) throws AlgorithmException {
        ...
        for (generation = 0; generation < limit && !end(); generation++) {
            ...
            try {
                lastPopulation = history[0];
                nextPopulation = history[this.historySize - 1];
                nextPopulation.setAs(lastPopulation);
    ...
    protected final void start(boolean reset) {
        ...
        if (reset || this.nextPopulation == null) {

            for (int i = 0; i < this.historySize; ++i) {
                this.history[i].setPool(this.initialPopulation.getPool());
            }

            this.nextPopulation = history[0];
            this.nextPopulation.setAs(this.initialPopulation);
            if (this.randomization > 0) {
                this.randomizePopulation(this.nextPopulation);
            }
        }
        this.lastPopulation = null;
        this.currentPopulation = this.nextPopulation;

        this.evaluatePopulation(this.nextPopulation);
    ...
    public final void evaluatePopulation(final Population<T> population, final boolean forced) {
        ...
        this.getFitness().prepare(population);
    ...
    protected final void applyElitism() {
        int currentSize = lastPopulation.size();
        ...
        if (fit != null) {
            fit.sort(lastPopulation);
        } else {
            Fitness.sort(lastPopulation, this.isBiggerBetter());
        }
        
        Individual<T> elite[] = new Individual[len];
        ...
        if (this.elitismStrategy == ElitismStrategy.WORST) {
            ...
            for (int i = 0; i < len; i++) {
                elite[i] = nextPopulation.getIndividual(nextSize - i - 1);
                elite[i].setAs(lastPopulation.getIndividual(i));
            }
        } else {
            // ElitismStrategy.RANDOM
            for (int i = 0; i < len; i++) {
                elite[i] = nextPopulation.getIndividual(random.nextInt(nextSize));
                elite[i].setAs(elite[i]);
            }
        }
        
        if( len > 0 ) {
            this.getFitness().adjust(this.nextPopulation, elite);
            for( Individual<T> ind: this.nextPopulation ) {
                this.getFitness().evaluate(ind);
            }
        }
