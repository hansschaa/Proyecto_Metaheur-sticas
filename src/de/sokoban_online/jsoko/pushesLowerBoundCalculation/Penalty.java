 ...
 */
package de.sokoban_online.jsoko.pushesLowerBoundCalculation;
...
public class Penalty {
	...
	private final int MAX_BOARD_SIZE_FOR_PENALTY_CALCULATION = 4900;
    ...
    private final ArrayList<BitSet> penaltySituations;
    ...
    public Penalty(Board board) {
		...
		penaltySituations = new ArrayList<>(10);

		if(board.size > MAX_BOARD_SIZE_FOR_PENALTY_CALCULATION) {	// to avoid bad performance in huge levels
			return;
		}
	 ...
	 */
    private void identifyPotentialPenaltySquares(){
                ...
                if(!board.isWallOrIllegalSquare(neighborSquarePosition)
                   ...
                   (!board.isGoal(position) ||
                    ...
                    do {
                        ...
                        if(!isPushWithoutLowerBoundIncreasePossible(position, UP) &&
                        ...
                        for(int playerPosition = board.firstRelevantSquare; playerPosition < board.lastRelevantSquare; playerPosition++) {
                            if(!board.playersReachableSquaresOnlyWalls.isSquareReachable(playerPosition) &&
                                    !board.isOuterSquareOrWall(playerPosition)) {
                    ...
                    if(boardPosition != null && isBoardPositionAtLeastOneTimeNoPenalty) {
	 ...
	 */
	private boolean isPushWithoutLowerBoundIncreasePossible(final int boxPosition, final int direction) {
	 ...
	 */
	private void identifyBoxRelevantPositions() {
	    ...
	    for(int position = board.firstRelevantSquare; position < board.lastRelevantSquare; position++) {
	        if(!board.isOuterSquareOrWall(position) && !board.isWallOrIllegalSquare(position)) {
	...
	final public int calculatePenalty() {
		...
		for (BitSet situation : penaltySituations) {
			penaltySituation = situation;
			...
			for (int index = 0; index < penaltySituation.size() - 1; index++) {
				if (!penaltySituation.get(index)) {
					continue;
				}
				if (!currentSituation.get(index)) {
			...
			if (penaltySituation.get(penaltySituation.size() - 1) ||
					isAFrozenBoxInSituation) {
				...
				if (penaltySquaresPositions[1] - penaltySquaresPositions[0] < board.width) {
					if (isPushWithoutLowerBoundIncreasePossible(penaltySquaresPositions[0], UP) ||
							isPushWithoutLowerBoundIncreasePossible(penaltySquaresPositions[1], UP)) {
						continue nextPenaltySituation;
					}
				} else {
					// Prüfen, ob das vertikale Penalty aufgelöst werden kann.
					if (isPushWithoutLowerBoundIncreasePossible(penaltySquaresPositions[0], RIGHT) ||
							isPushWithoutLowerBoundIncreasePossible(penaltySquaresPositions[1], RIGHT)) {
						continue nextPenaltySituation;
					}
				}
			}
	    ...
	    // Display penaly value if requested
	    if(Debug.debugShowPenaltyFields) {
	    	Debug.debugApplication.applicationGUI.mainBoardDisplay.displayInfotext("Penalty = "+penalty);
	    }
	...
	public void debugShowPenaltySituations() {
		...
		for (BitSet situation : penaltySituations) {
			...
			// Falls gewünscht werden zu Debugzwecken alle Penaltysituationen einzeln angezeigt.
			if (Debug.debugShowPenaltySituationsSeparately) {
				Debug.debugApplication.redraw(true);
				board.removeAllMarking();
			}
		}

	    // Spielfeld mit Markierungen ausgeben
