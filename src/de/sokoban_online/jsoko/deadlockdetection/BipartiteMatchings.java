...
/**
 * Class for calculating minimum number of pushes needed to push every box to a goal.
 * <p>
...
public class BipartiteMatchings {
	 ...
	 * necessarily mean there is no deadlock.
	 *
	 * Example:<pre>
	 * #######
	 * # $.$ #
	 ...
	 * @return <code>true</code> if the board is a deadlock, or <code>false</code> if no deadlock has been found
	 */
	public boolean isDeadlock(SearchDirection searchDirection, boolean[] isGoalExcluded) {
	 ...
	 */
	private boolean calculateBenefits(SearchDirection searchDirection, boolean isDeadlockDetection) {
	 ...
	 */
	private int getDistance(int boxNo, int goalNo, SearchDirection searchDirection) { 
	 ...
	 * @return {@code true} perfect matching might be possible, {@code false} no perfect matching possible => deadlock
	 */
	private boolean calculateBenefitsForDeadlockDetection(SearchDirection searchDirection, boolean[] isGoalExcluded) {
	...
	private boolean searchPerfectBipartiteMatchingWithHighestBenefit() {
		...
		minimumNoDeadlockValue = - (2L * board.boxCount - 1) * highestAbsoluteBenefit - ( board.boxCount - 1L) * epsilon;
