...
public class ClosedDiagonalDeadlock implements DirectionConstants {
	 ...
	 *   This however mustn't be done beginning from the pushed box position but
		...
		while(true) {
			...
			currentDiagonal:
				while(true) {
					...
					if(board.isOuterSquareOrWall(currentDiagonalSquarePosition) && !board.isWall(currentDiagonalSquarePosition)) {
					...
					while(true) {
						...
						if(!board.isBoxOrWall(diagonalSquareNeighbor1Position)) {
							break currentDiagonal;
						}
						...
						if(!board.isBoxOrWall(currentDiagonalSquarePosition)) {
							...
							if(!board.isBoxOrWall(diagonalSquareNeighbor2Position)) {
								break currentDiagonal;
							}
							...
							if(!board.isWall(diagonalSquareNeighbor2Position) || !board.isWall(currentDiagonalSquarePosition + dy)) {
						...
						if (!isStartingSquareOneEndOfTheDiagonal && dy == board.offset[UP]) {
