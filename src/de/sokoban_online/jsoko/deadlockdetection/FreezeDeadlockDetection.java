...
public final class FreezeDeadlockDetection {
	...
	private int   timestamp = 2;		// must be even and not zero
	private final int[] freezeValues;
	...
	/**
	 * Returns whether the current board position is a "freeze" deadlock.
	 * <p>
	 * This method checks if a box is blocked by other boxes / walls in the way
	 * that it can never be pushed again. If this is the case and the box isn't
	 * located at a goal then there is a deadlock.<br>
	 * If there isn't a deadlock but boxes are frozen (= immovable) then the
	 * frozen boxes are marked in the board.
	 *
	...
	public boolean isDeadlock(int boxPosition, boolean setFrozenBoxes) {
		...
		if (checkBoxPushability(boxPosition, HORIZONTAL)) {
			return false;
		}
		...
		// Check vertical axis.
		if (checkBoxPushability(boxPosition, VERTICAL)) {
			return false;
		}
	 ...
	 */
	private boolean checkBoxPushability(final int boxPosition, int axisdir) {
		...
		if (isBoxPushable || board.isGoal(boxPosition)) {
			...
			if (!isABoxFrozenOnANotGoalSquare && board.isBox(boxNeighbor2)) {
		...
		if (isBoxPushable
			...
			if (       !board.isSimpleDeadlockSquare(boxNeighbor1)
			...
			if (!isBoxPushable
					&& isOtherDirectionAccessible
					&& !board.isSimpleDeadlockSquare(boxNeighbor2)) {
				isBoxPushable = true;
			}
		}
