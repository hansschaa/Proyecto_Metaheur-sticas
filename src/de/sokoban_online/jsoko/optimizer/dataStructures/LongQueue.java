...
/**
 * Unbounded FIFO queue for storing <code>Long</code>s >= 0.
 * <p>
 * This queue only stores positive longs ( >= 0)!
 */
public class LongQueue {
	...
	/** This is the number of longs stored in one memory block. */
	...
	/**
	 * Creates an unbounded queue for storing <code>long</code>s >= 0.
	 * <p>
	 * This queue can only store values >= 0.<br>
	 * If more than {@code memoryBlockSize} longs are stored,
	 * several memory blocks are chained in a linked list so all longs can be stored. <br>
	 ...
	 *
	 * @param memoryBlocksSize  number of longs stored in one memory block
	 */
	public LongQueue(int memoryBlocksSize) {

		// Set the number of longs stored in one memory block.
		...
		memoryBlockToWrite = new AtomicReference<>(new MemoryBlock(MEMORY_BLOCK_SIZE));
		memoryBlockToRead  = new AtomicReference<>(memoryBlockToWrite.get());
	}

	/**
	 * Adds the passed value to this queue.
	 * <p>
	 * If an <code>long</code> < 0 is passed the method returns
	...
	public void add(long value) {
		...
		// Therefore 0 is saved using the Long.MIN_VALUE which can't occur as value.
		if(value == 0) {
	...
	public long remove() {
		...
		while(true) {
			...
			while((indexToRead = memoryBlock.nextIndexToRead.get()) < MEMORY_BLOCK_SIZE) {
				...
				long value = memoryBlock.get(indexToRead);
				if(value == EMPTY) {
					return NONE;
				}
	...
	public void debugTest(final int threadCount) {
		...
		for(int i=0; i<1000; i++) {
			...
			final AtomicLongArray hashtable  = new AtomicLongArray(testNumbersCount*threadCount);
				...
				threads[id] = new Thread(() -> {
					for(int i1 = 0; i1 <testNumbersCount; i1++) {
						...
						if(i1 %3 == 0) {
							long value = remove();
							if(value != NONE) {
hashtable.incrementAndGet((int) value);
}
						}
					}
...
break;
	...
	/**
	 * Memory block used in the queue for <code>long<code>s.
	 * <p>
	 * This essentially is a long array which additionally holds a reference
	...
	static class MemoryBlock extends AtomicLongArray {
		...
		final AtomicReference<MemoryBlock> nextMemoryBlock = new AtomicReference<>();
